...
const BRIDGE_COMMANDS = [
  ...
  {
    command: "ccn",
    description: "Create new Claude session and send instruction",
  },
...
async function onboard() {
  ...
  console.log();
  console.log(
    `  ${chalk.bold.cyan("openclaw-claude-bridge")} ${chalk.dim(`v${pkg.version}`)}`,
  );
  ...
  console.log(
    `  ${chalk.bold("[1/4]")} ${chalk.bold("Checking dependencies...")}`,
  );
  console.log();
  await sleep(200);
  ...
  for (const dep of results) {
    ...
    if (cmd) {
      ...
      console.log(
        `      ${chalk.yellow("!")} No package manager found. Install tmux manually.`,
      );
      missing.push(dep);
    }
  }
  ...
  console.log(
    `  ${chalk.bold("[2/4]")} ${chalk.bold("Detecting environment...")}`,
  );
  console.log();
  ...
  console.log();
  console.log(
    `      The target ID identifies ${chalk.bold("who receives")} Claude's responses.`,
  );
  console.log(`      For example:`);
  console.log(
    `        ${chalk.dim("\u2022")} Telegram  ${chalk.dim("->")} your chat ID ${chalk.dim("(e.g. 123456789)")}`,
  );
  console.log(
    `        ${chalk.dim("\u2022")} Discord   ${chalk.dim("->")} channel ID ${chalk.dim("(e.g. 123456789012345678)")}`,
  );
  console.log(
    `        ${chalk.dim("\u2022")} Slack     ${chalk.dim("->")} channel name ${chalk.dim("(e.g. #general)")}`,
  );
  console.log();
  console.log(
    `      ${chalk.cyan.bold("Opening your OpenClaw channels page to check...")}`,
  );

  const opened = openBrowser(CHANNELS_URL);
  if (opened) {
    console.log(
      `      ${chalk.green("\u2713")} Browser opened: ${chalk.underline(CHANNELS_URL)}`,
    );
    console.log(
      `      ${chalk.dim("Find your channel and copy the target ID from the dashboard.")}`,
    );
  } else {
    console.log(
    ...
    console.log(
      `      ${chalk.dim("Open this URL manually:")} ${chalk.underline(CHANNELS_URL)}`,
    );
  }
  ...
  try {
    ...
    if (!config.plugins.allow.includes("claude-bridge")) {
      config.plugins.allow.push("claude-bridge");
    }

    // Plugin entry config
    if (!config.plugins.entries) config.plugins.entries = {};
    config.plugins.entries["claude-bridge"] = {
      enabled: true,
      config: {
    ...
    config.plugins.installs["claude-bridge"] = {
      source: "path",
  ...
  const legacyHookDir = path.join(workspace, "hooks", "claude-bridge");
  ...
  if (fs.existsSync(claudeMdDest)) {
    ...
    spinner3.stop(
      `CLAUDE.md   ${chalk.dim("->")} existing backed up as ${chalk.yellow(path.basename(backupPath))}`,
    );
