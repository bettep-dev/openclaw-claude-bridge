...
async function onboard() {
  ...
  for (const file of scriptFiles) {
    ...
    if (fs.existsSync(dest)) {
      ...
      copyTemplate(path.join('scripts', file), dest, vars);
      fs.chmodSync(dest, 0o755);
      scriptsCreated++;
    }
  }

  const scriptMsg = scriptsPatched > 0 && scriptsCreated > 0
    ? `Scripts     ${chalk.dim('->')} ${scriptsDir} ${chalk.dim(`(${scriptsCreated} created, ${scriptsPatched} updated)`)}`
    : scriptsPatched > 0
    ? `Scripts     ${chalk.dim('->')} ${scriptsDir} ${chalk.dim(`(${scriptsPatched} updated)`)}`
    : `Scripts     ${chalk.dim('->')} ${scriptsDir} ${chalk.dim(`(${scriptsCreated} created)`)}`;
  spinner1.stop(scriptMsg);
  ...
  const skillMsg = skillsPatched > 0 && skillsCreated > 0
    ? `Skills      ${chalk.dim('->')} ${skillsDir} ${chalk.dim(`(${skillsCreated} created, ${skillsPatched} updated)`)}`
    : skillsPatched > 0
    ? `Skills      ${chalk.dim('->')} ${skillsDir} ${chalk.dim(`(${skillsPatched} updated)`)}`
    : `Skills      ${chalk.dim('->')} ${skillsDir} ${chalk.dim(`(${skillsCreated} created)`)}`;
  spinner2.stop(skillMsg);
