...
const { detectOS, getHomeDir, getWorkspace, getScriptsDir, installDaemon, substituteTemplate } = require('./platform');
...
async function onboard() {
  ...
  try {
    ...
    console.log(`    ${chalk.yellow('!')} Could not write openclaw.json — plugin may need manual config`);
  }
  ...
  spinner2a.stop(formatInstallMsg('Plugin', pluginDir, pluginCreated, pluginPatched));
  ...
  if (skillsRemoved > 0) {
    spinner2.stop(`Skills      ${chalk.dim('->')} ${skillsRemoved} legacy skills removed ${chalk.dim('(replaced by hook)')}`);
  } else {
    spinner2.stop(`Skills      ${chalk.dim('->')} no legacy skills found`);
  }
  ...
  if (restarted) {
    ...
    spinner5.fail(`Gateway     ${chalk.dim('->')} could not restart ${restartError ? chalk.dim(`(${restartError})`) : ''}`);
  }
  ...
  console.log();
  console.log(`  ${chalk.dim('Prefix @cc and /cc are both supported. Quotes are optional.')}`);
  console.log();
}
