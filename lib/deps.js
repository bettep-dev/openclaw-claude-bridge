...
function printStatus(results) {
  for (const dep of results) {
    if (dep.found) {
      console.log(`    ${chalk.green('\u2713')} ${dep.name.padEnd(10)} ${chalk.dim(dep.path)}`);
    } else {
      const cmd = dep.install || (process.platform === 'darwin' ? dep.macOS : dep.linux);
      console.log(`    ${chalk.red('\u2717')} ${dep.name.padEnd(10)} ${chalk.red('not found')}`);
...
module.exports = { DEPS, checkAll, printStatus, whichBin, getInstallCmd, installDep };
